...
parser.add_argument('--early_stopping', default=False, action='store_true', help='Save the model only if current validation error < minimum validation error.')
parser.add_argument('--iterations', type=int, default=10000, help='The number of training iterations')
...
def main():
    ...
    with tf.Session() as sess:
        ...
        try:
            while counter < args.iterations + 1:
                if counter % args.save_every == 0 and not args.early_stopping:
                    save()
                if counter % args.sample_every == 0:
                    generate_samples()
                if args.val_every > 0 and (counter % args.val_every == 0 or counter == 1):
                    current_val_loss = validation()
                    if args.early_stopping and current_val_loss < min_val_loss:
                        save()
